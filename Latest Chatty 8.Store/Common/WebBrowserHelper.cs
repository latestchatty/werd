...
<head>
	...
	<script type='text/javascript'>
		...
		function toggleEmbeddedVideo(container, url) {
			try {
				var target = container.getElementsByTagName('div')[0];
				...
				if (embed === undefined) {
					...
					video.onloadedmetadata = function () { window.external.notify(JSON.stringify({'eventName': 'imageloaded', 'eventData': {}})); };
					...
					window.external.notify(JSON.stringify({'eventName': 'debug', 'eventData': {'name': 'Embed Video', 'width': width, 'url': url }}));
				} else {
