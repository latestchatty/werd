...
<script>
		function addPlaceholderImage(target) {
			...
			placeholder.onload = function () { emitMessage('resizeRequired'); };
			...
			placeholder.onclick = function () { target.removeChild(placeholder); emitMessage('resizeRequired'); }
			return placeholder;
		}
</script>";
...
function toggleEmbeddedImage(container, url) {
			try {
				...
				emitMessage('error', {'errorInfo': err, 'url': url});
				return false;
			}
		}
</script>";
	...
	function toggleEmbeddedVideo(container, urls) {
		try {
			...
			if (embed === undefined) {
				...
				video.onloadedmetadata = function () { target.removeChild(placeholder); emitMessage('resizeRequired'); };
				target.onclick = function (e) { target.removeChild(video); emitMessage('resizeRequired'); };
				...
				emitMessage('debug', {'name': 'Embed Video', 'width': width, 'urls': urls });
			} else {
			...
			emitMessage('error', {'errorInfo': err, 'urls': urls});
			return false;
		}
	}
</script>";
...
<script>
		function toggleEmbeddedTwitter(container, tweetId) {
			try {
				...
				if (alreadyEmbedded === undefined) {
					...
					twttr.widgets.createTweet
						...
						.then(function(el) {
							target.removeChild(placeholder);
							emitMessage('resizeRequired');
							emitMessage('debug', {'name': 'Embeded Tweet Loaded', 'tweetId': tweetId});
						});
				} else {
					target.removeChild(alreadyEmbedded);
					emitMessage('resizeRequired');
				...
				emitMessage('error', {'errorInfo': err, 'tweetId': tweetId});
				return false;
			}
		}
</script>";
...
function toggleEmbeddedYoutube(container, ytId) {
	try {
		...
		if (alreadyEmbedded === undefined) {
			...
			emitMessage('resizeRequired');
			emitMessage('debug', {'name': 'Embeded Youtube Loaded', 'ytId': ytId});
		} else {
			target.removeChild(alreadyEmbedded);
			emitMessage('resizeRequired');
		}
		...
		emitMessage('error', {'errorInfo': err, 'ytId': ytId});
		return false;
	}
}
function launchExternalYoutube(ytId) {
	emitMessage('externalYoutube', {'ytId': ytId});
	return false;
}
</script>";
